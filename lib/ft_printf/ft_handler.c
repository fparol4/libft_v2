...
int	ft_handle_unsigned(unsigned int value)
{
	...
	b_dec = "0123456789";
	v_str = ft_itoa_unsigned(value, b_dec);
	return (ft_putstr(v_str, 1));
}
...
int	ft_handle_hex(unsigned int v, int upper)
{
	...
	if (upper)
		b_hex = "0123456789ABCDEF";
	v_str = ft_itoa_unsigned(v, b_hex);
	return (ft_putstr(v_str, 1));
}
...
int	ft_handle_ptr(size_t address)
{
	...
	b_hex = "0123456789abcdef";
	v_str = ft_itoa_unsigned((size_t) address, b_hex);
	ft_putstr("0x", 0);
	return (ft_putstr(v_str, 1) + 2);
}
